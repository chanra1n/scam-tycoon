...
function runIdentityTheftScam() {
  ...
  const person = {
    class: isPoor ? "poor" : isRich ? "rich" : "middle",
    money: Math.floor(Math.random() * (isPoor ? 2000 : isRich ? 50000 : 10000) + (isPoor ? 500 : isRich ? 20000 : 5000)),
    defense: Math.floor(Math.random() * (isPoor ? 20 : 50)) + 1 // Increased maximum defense score
  };
  ...
  if (scamSucceeded) {
    const cut = person.money * 0.8; // Higher cut of the person's money
